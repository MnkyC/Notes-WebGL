...
function main() {
  ...
  var tick = function () {
    ...
    animateID = requestAnimationFrame(tick, canvas); // 请求浏览器调用tick Request that the browser calls tick
  };
  tick();

  // setTimeout(() => {
  //   cancelAnimationFrame(animateID); // 请求浏览器停止调用tick Request that the browser stop calling tick
  // }, 10000);
}
...
function draw(gl, n, currentAngle, modelMatrix, u_ModelMatrix) {
  ...
  modelMatrix.setRotate(currentAngle, 0, 0, 1); // 旋转角度和旋转轴 Rotation angle, rotation axis (0, 0, 1)
